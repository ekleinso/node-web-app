...
This git repo is a simple contains a simple node application that uses argocd to deploy its tekton pipeline to OpenSHift 4.3.  It the uses Tekton to build an image, publish it to the Container Registry and executes an argo sync to deploy the app.  
...
1. To run the application locally, [Install Docker Desktop](https://www.docker.com/products/docker-desktop) or you could use [podman](https://podman.io/) on Linux (This assumes you use podman CLI instead of docker.  Substitute docker <command> with podman <command>)
...
### Using OpenShift 4.6 as my Kubernetes Cluster

You need your own 4.6 OpenShift Cluster.  Here are some options.  

I [installed OpenShift 4.6 into AWS following these instructions](https://docs.openshift.com/container-platform/4.3/installing/installing_aws/installing-aws-account.html).
...
- [oc](https://docs.openshift.com/container-platform/4.6/cli_reference/openshift_cli/getting-started-cli.html#installing-the-cli)
...
Create an instance of argocd this is taken from [the examples](https://github.com/argoproj-labs/argocd-operator/tree/master/examples). The following example creates argocd with OpenShift routes.
```
...
Retrieve the server secret key and save for later
```
oc -n argocd get secret argocd-secret -o jsonpath='{.data.server\.secretkey}' | base64 -d
```

and set the admin password
```
oc -n argocd patch secret argocd-cluster -p '{"stringData": {"admin.password": "yourpasswordhere"}}'
```
...
argocd cluster add $(oc config get-contexts -o name | awk '/^argocd/{print $1}')
```
...
There is a file called node-web-app-argocdsecret.template.  Run the following command to create the ConfigMap and Secret based on that template file.


```
oc apply -n node-web-project -f - <<EOF
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  ARGOCD_USERNAME: $(echo -n admin | base64 -w0)
  ARGOCD_PASSWORD: $(echo -n yourpassword | base64 -w0)
EOF
```
...
!!!! Be aware that these resources may change with newer versions. As was the case from OpenShift 4.3 to OpenShift 4.6

[node-web-app-pipeline-resources.yaml](pipeline/node-web-app-pipeline-resources.yaml): [Pipeline Resources](https://github.com/tektoncd/pipeline/blob/master/docs/resources.md) configured for the pipeline.  There are 2, the name of the git repository and the name of the Container Image using the Internal Red Hat Registry.  Note, the resources here allow us to do a Pipeline Run from the console or oc command line.  It hard codes default values.  They will be overridden by Trigger Template when builds are done via a git push.  

[node-web-app-pipeline.yaml](pipeline/node-web-app-pipeline.yaml): Our [Pipeline](https://github.com/tektoncd/pipeline/blob/master/docs/pipelines.md) for building, publishing, and deploying our Node App.  There are 2 [Tasks](https://github.com/tektoncd/pipeline/blob/master/docs/tasks.md).  We make use of some default tasks rather than creating our own.  A real life pipeline will execute tests, tag images, and so forth.  Tasks:

- the build-and-publish-image uses the ClusterTask buildah-pr (podman build system).  
- argocd-sync-deployment uses the argocd ClusterTask we installed earlier
...
- A TriggerTemplate is used to create a template of the same pipeline resources, but dynamically generated to not hard code image name or source.  It also creates a PipelineRun Template that will be created when a build is triggered.

- A TriggerBinding that binds the incoming event data to the template (this will populte things like git repo name, revision,etc....)
...
- Application Name: node-web-app-pipeline
- Project: default
- AutoSync: Enabled
- Repo URL: https://github.com/your-repo/node-web-app.git
- Revision: Head
- PATH: pipeline
...
3. The pipeline should run, the build should pass (Creates the Container Image and publishes it to the Container Registry).  The argo-cd sync should fail because we have not configured the argocd app for deploying the node-web-project.  
...
| Field | Value |
|-------|-------|
| Payload URL | http://$(oc -n argocd get route argocd-server -o jsonpath='{.spec.host}')/api/webhook |
| Content Type | application/json |
| Secret | **specify your passphrase** |
| Trigger Events | push |
...
| Field | Value |
|-------|-------|
| Payload URL | http://$(oc -n node-web-project get route el-node-web-app -o jsonpath='{.spec.host}') |
| Content Type | application/json |
| Secret | **specify your passphrase** |
| Trigger Events | push |
